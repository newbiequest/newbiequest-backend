...
public class ChatService {
    ...
    @Transactional
    public ChatResponse getAnswer(Long memberId) {
        ...
        OpenAiResponse openAiResponse = openAiClient.getChatCompletion(candidates, member.getNickname());
        String contnet = openAiResponse.getChoices().get(0).getMessage().getContent();

        try {
            return objectMapper.readValue(contnet, ChatResponse.class);
        } catch (Exception e) {
